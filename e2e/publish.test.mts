...
for (const [{ payload }, pr] of [[pushWorkflowJobQueuedFixture], [prWorkflowJobQueuedFixture, prPullRequestSynchronizeFixture]] as const) {
  {
    // workflow_job.queued
...

  {
    const env = Object.entries({
      ...
      GITHUB_REF_NAME: pr ? `${pr.payload.number}/merge`: payload.workflow_job.head_branch,

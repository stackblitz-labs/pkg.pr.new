...
const fetchRepositories = async (): Promise<void> => {
  try {
    ...
    if (responseData.error) {
      logToClient(
        `Error fetching repositories: ${responseData.message || "Unknown error"}`,
        "error",
...
<template>
  <div>
    <div
      class="my-container flex flex-col items-center gap-4 md:gap-8 min-h-[calc(100vh-80px)]"
    >
      ...
      <div class="w-full max-w-4xl mt-4">
        <h2 class="text-xl font-semibold mb-4">
          Available Repositories in R2 Storage
        </h2>

        <div v-if="loading" class="p-4 text-center">
          <div
            class="animate-spin rounded-full h-10 w-10 border-b-2 border-primary-500 mx-auto"
          ></div>
        ...
        <div
          v-else-if="error"
          class="p-4 border border-red-300 bg-red-50 dark:bg-red-900/20 dark:border-red-800 rounded-lg"
        >
          <p class="text-red-600 dark:text-red-400">{{ error }}</p>
          <button
            @click="fetchRepositories"
            class="mt-2 px-4 py-2 bg-primary-500 text-white rounded hover:bg-primary-600 transition"
          >
            Try Again
          </button>
        </div>
          ...
          >
            ...
            <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">
              ...
              <div class="mt-2">
                ...
                <div
                  v-if="repo.latest_commit"
                  class="bg-gray-50 dark:bg-gray-800 p-2 rounded mt-1"
                >
                  <div class="flex gap-2 items-center">
                  ...
                  <p class="text-xs mt-1">
                    {{ truncate(repo.latest_commit.message, 80) }}
                  </p>
                  <p class="text-xs mt-1 text-gray-400">
                    {{ formatDate(repo.latest_commit.date) }}
                  </p>
                </div>
                <p v-else class="text-yellow-600 dark:text-yellow-400 italic">
                  No commits found
                </p>
              </div>
      ...
      <div class="fixed bottom-4 right-4 z-50">
        ...
        >
          ...
          <span class="bg-red-500 rounded-full px-2 text-xs">{{
            clientLogs.length
          }}</span>
        </button>
        ...
        <div
          v-if="showLogs"
          class="mt-2 bg-gray-800 text-white p-4 rounded-lg w-96 max-h-96 overflow-y-auto shadow-xl"
        >
          ...
          <div
            v-for="(log, index) in clientLogs"
            :key="index"
            :class="`mb-2 p-2 rounded text-xs ${
              log.type === 'error'
                ? 'bg-red-900/50 border-l-2 border-red-600'
                : log.type === 'success'
                  ? 'bg-green-900/50 border-l-2 border-green-600'

...
export class R2GitHubService {
  ...
  constructor(storage: ReturnType<typeof useStorage>, debugEnabled = true, event?: H3Event) {
  ...
  // Repository methods
  async getRepository(owner: string, repo: string): Promise<R2Repository | null> {
    const key = `${REPO_PREFIX}${owner}/${repo}`;
    this.log(`Attempting to fetch repository ${owner}/${repo} from R2 storage with key "${key}"`);
    ...
    try {
      ...
      if (data) {
        ...
        this.log(`Repository details: name=${parsedData.name}, default_branch=${parsedData.default_branch}, indexed_at=${parsedData.indexed_at}`);
        return parsedData;
      } else {
        this.log(`Repository ${owner}/${repo} not found in R2 storage (key: ${key})`);
        return null;
      }
    } catch (error) {
      this.log(`Error fetching repository ${owner}/${repo} from R2 storage: ${error}`, true);
      return null;
    }
  }
  ...
  async storeRepository(repository: R2Repository): Promise<void> {
    ...
    this.log(`Storing repository ${repository.owner.login}/${repository.name} in R2 storage with key: ${key}`);
    await this.storage.setItem(key, JSON.stringify(repository));
    
    // Update search index
    await this.updateSearchIndex('repositories', [
      repository.name,
  ...
  async searchRepositories(query: string, limit = 10): Promise<R2Repository[]> {
    ...
    try {
      ...
      this.log(`Retrieved search index with ${Object.keys(searchIndex.repositories).length} terms`);
      ...
      for (const term of terms) {
        ...
        this.log(`Term "${term}" matched ${matchingTerms.length} indexed terms`);
      ...
      this.log(`Found ${repoIds.size} unique repositories matching the search terms`);
        ...
        if (key.startsWith("repo:")) {
          ...
          if (data) {
            ...
            if (repoIds.has(repo.id)) {
              this.log(`Adding repository to search results: ${repo.owner.login}/${repo.name}`);
              repos.push(repo);
      ...
      this.log(`Returning ${repos.length} repositories matching search query "${query}"`);
      return repos;
    } catch (error) {
  ...
  // Commit methods
  async getCommit(owner: string, repo: string, sha: string): Promise<R2Commit | null> {
  ...
  async storeCommit(owner: string, repo: string, commit: R2Commit): Promise<void> {
    ...
    // Update search index
    await this.updateSearchIndex('commits', [
      commit.sha,
  ...
  async listCommits(owner: string, repo: string, page = 1, per_page = 10): Promise<R2Commit[]> {
    this.log(`Listing commits for repository ${owner}/${repo} (page=${page}, per_page=${per_page}) from R2 storage`);
    
    try {
      // Get repository first to verify it exists
      const repository = await this.getRepository(owner, repo);
      if (!repository) {
        this.log(`Cannot list commits: Repository ${owner}/${repo} not found in R2 storage`, true);
        return [];
      }
      ...
      this.log(`Found ${commitKeys.length} total commits for ${owner}/${repo} in R2 storage`);
      ...
      this.log(`Returning ${paginatedCommits.length} commits for page ${page} (offset ${start})`);
      if (paginatedCommits.length > 0) {
        this.log(`First commit in response: ${paginatedCommits[0].sha.substring(0, 7)} - "${paginatedCommits[0].commit.message.split('\n')[0]}"`);
        this.log(`Last commit in response: ${paginatedCommits[paginatedCommits.length - 1].sha.substring(0, 7)} - "${paginatedCommits[paginatedCommits.length - 1].commit.message.split('\n')[0]}"`);
      }
  ...
  // Check Run methods
  async getCheckRuns(owner: string, repo: string, commitSha: string): Promise<R2CheckRun[]> {
    const keys = await this.storage.getKeys();
  ...
  async storeCheckRun(owner: string, repo: string, commitSha: string, checkRun: R2CheckRun): Promise<void> {
  ...
  private extractKeywords(terms: string[]): string[] {
    ...
    return keywords.filter(word => word.length > 2 && !stopWords.includes(word));
  }
  ...
  async indexCommits(owner: string, repo: string, maxCommits = 100): Promise<void> {
    try {
      const octokit = useGithubREST(this.event);
  ...
  async indexCheckRuns(owner: string, repo: string, commitSha: string): Promise<void> {
    try {
      const octokit = useGithubREST(this.event);
      ...
      console.log(`Indexed ${checkRunsData.check_runs.length} check runs for commit ${commitSha}`);
    } catch (error) {
      console.error(`Error indexing check runs for commit ${commitSha}:`, error);

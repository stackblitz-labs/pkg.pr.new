...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.log(
      `[R2API] Fetching commits for ${query.owner}/${query.repo} (page=${query.page}, per_page=${query.per_page})`,
    );
    ...
    console.log(
      `[R2API] Found repository in R2: ${query.owner}/${query.repo} (id: ${repository.id})`,
    );

    // Get commits from R2 storage
    console.log(
      `[R2API] Fetching commits from R2 storage for ${query.owner}/${query.repo}`,
    );
    ...
    if (commits.length === 0) {
      console.log(
        `[R2API] No commits found for ${query.owner}/${query.repo} in R2 storage`,
      );
      return {
        data: [],
    ...
    const responseData = commits.map((commit) => {
      console.log(
        `[R2API] Processing commit: ${commit.sha.substring(0, 7)} - "${commit.commit.message.split("\n")[0]}"`,
      );
